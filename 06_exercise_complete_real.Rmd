...
Groceries_dataset_2  <- read_csv("Groceries_dataset.csv")
...
tab2 <-
  tab %>% 
  tab_header(
    title = md("Lisa's Garden data, sorted")
  )
tab2
...
fall2017_classdata <- read_html("https://www.macalester.edu/registrar/schedules/2017fall/class-schedule/#crs10008")
```
...
```
  ...
  * Day
```{r}
course_days <- fall2017_classdata %>%
...
```
  ...
  * Time
```{r}
course_times <- fall2017_classdata %>% 
...
```
  ...
  * Room
```{r}
course_rooms <- fall2017_classdata %>% 
...
```
  ...
  * Instructor
```{r}
course_instructor <- fall2017_classdata %>% 
...
```
  ...
  * Avail. / Max
```{r}
course_avail <- fall2017_classdata %>% 
...
```{r}
course_df <- tibble(Number = course_numb, 
                    Name = course_names,
                    Day = course_days,
                    Time = course_times,
                    Room = course_rooms,
...
```{r, fig.height=6}
course_df %>% 
  mutate(Number = str_sub(Number, end = -7)) %>% 
  group_by(Number) %>% 
  count(Number) %>% 

  ggplot() +
  geom_col(aes(x = n,
               y = fct_rev(fct_infreq(Number)), 
               fill = n)) +
  
  labs(title = "Sections Offered Per Department",
       x = "",
       y = "Department(abbreviated)",
       caption = "Graph by Jon Kazor; Data from Macalester College Registrar.")
...
```{r}
name_length <- 
str_length(course_names)

descrip_length <-
str_length(descrip)

Dept_sections <- course_numb %>%
str_sub(start = 1, end = -7)

course_df_1 <- tibble(Department = Dept_sections,
Number = course_numb,
Name = course_names,
Day = course_days,
Time = course_times,
...
Course_Length = descrip_length)
head(course_df_1)

course_df_1 %>%
group_by(Department) %>% 
filter(n() >= 10 ) %>%
mutate(Avg_length = mean(Name_Length)) %>% 
ggplot() +
geom_bar(aes(y = Department, 
fill = Avg_length)) +
scale_x_continuous(expand=c(0,0)) +
labs(title = "Average Length of Course Name for Classes, by Department",
x = "Count of Classes in the Dept",
fill = "Average Title Length")
```
